/*
 * muos_audio_ghost.js — v13: IPC nativo, PLAYPAIR, OnEnded e SFX polifônicos
 *
...
(function () {
  ...
  try { console.log(MUTE ? "MUOS_AUDIO_GHOST_V13_MUTE" : "MUOS_AUDIO_GHOST_V13_IPC"); } catch (e) {}
  ...
  // Algumas trilhas antigas nao seguem foo_intro -> foo_loop. Sem este mapa o intro
  // curto dependia exclusivamente do OnEnded JS e a batalha podia ficar em silencio.
  var MUSIC_PAIR_OVERRIDES = {
  ...
  var endedConditionCache = {
    runtime: null,
    original: null,
    conditions: []
  };
  function getEndedConditions(rt, original) {
    if (endedConditionCache.runtime === rt && endedConditionCache.original === original &&
        endedConditionCache.conditions.length)
      return endedConditionCache.conditions;

    var found = [];
    if (rt.cndsBySid) for (var sid in rt.cndsBySid) {
      if (!rt.cndsBySid.hasOwnProperty(sid)) continue;
      var condition = rt.cndsBySid[sid];
      if (condition && condition.func === original) found.push(condition);
    }
    endedConditionCache.runtime = rt;
    endedConditionCache.original = original;
    endedConditionCache.conditions = found;
    return found;
  }

  // dispara o trigger "On <tag> ended" no runtime (a cena avanca por ele).
  // audTag e' closure-privado do plugin. O C2 tambem guarda OnEnded em Condition.func,
  // portanto o shim temporario precisa cobrir o prototype e as referencias compiladas.
  function fireEndedTag(rt, audioPlugin, tag) {
    try {
      ...
      var syntheticOnEnded = function (t) {
        return ("" + t).toLowerCase() === ("" + tag).toLowerCase();
      };
      var patchedConditions = getEndedConditions(rt, origOnEnded);
      for (var i = 0; i < patchedConditions.length; ++i)
        patchedConditions[i].func = syntheticOnEnded;
      cnds.OnEnded = syntheticOnEnded;
      try { rt.trigger(origOnEnded, audioPlugin); }
      finally {
        for (var j = 0; j < patchedConditions.length; ++j)
          patchedConditions[j].func = origOnEnded;
        cnds.OnEnded = origOnEnded;
      }
      return true;
  ...
  function installActWrappers() {
    try {
      ...
      function doPlay(self, name, isMusic, looping, volDb, tag) {
        ...
        if (!loop && tagLower === "musicintro") {
          var pairCand = MUSIC_PAIR_OVERRIDES[oneName];
          if (!pairCand && /_intro$/.test(oneName))
            pairCand = oneName.replace(/_intro$/, "_loop");
          if (pairCand && DUR.hasOwnProperty(pairCand)) pairLoopName = pairCand;
        }
      ...
      console.log("MUOS_ACTS_WRAPPED_V13");
      return true;
    } catch (e) {
