...
BUILD_PROFILE=${MOONRIDER_BUILD_PROFILE:-baseline}
OPT_CFLAGS=$("$SRC/scripts/build-flags.sh" "$BUILD_PROFILE")
echo "=== build profile: $BUILD_PROFILE ($OPT_CFLAGS) ==="
...
$CC $OPT_CFLAGS -fPIC -shared -Wl,-soname,libGL.so.1 \
